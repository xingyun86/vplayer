...
public class PlayerController {
    ...
    public void initialize(){
        //设置各控件图标
        setIcon(playBT,App.playIcon,25);
        setIcon(stopBT,App.stopIcon,25);
        setIcon(volumeBT,App.volOnIcon,15);
        setIcon(maxBT,App.maxIcon,25);
        setIcon(openBT,App.openIcon,25);
    ...
    public void start(String url,boolean popup,int width,int height){
        ...
        if(!url.isEmpty())
        {
            ...
            {
                videoUrl = "file:///" + this.url.replace('\\','/');            
            }
            //MediaView设置
    ...
    private void setMediaViewOnClick(){
        mediaView.setOnMouseClicked(event -> {
            ...
            if(status == MediaPlayer.Status.PAUSED || status == MediaPlayer.Status.READY || status == MediaPlayer.Status.STOPPED){
                ...
                mediaPlayer.play();
                setIcon(playBT,App.pauseIcon,25);
            }else{   //当资源处于播放状态时
                mediaPlayer.pause();
                setIcon(playBT,App.playIcon,25);
            }
    ...
    private void setOpenButton(){
        openBT.setOnAction((ActionEvent e)->{
            if(media == null)
            {        
                ...
                this.url = App.videoPath;
                String videoUrl = this.url;
                if(!this.url.startsWith("http"))
                {
                    videoUrl = "file:///" + this.url.replace('\\','/');            
                }
                ...
                mediaView.setMediaPlayer(mediaPlayer);
                //设置播放器，在媒体资源加载完毕后，获取相应的数据，设置组件自适应布局
                setMediaPlayer(currentWidth,currentHeight);
            }
        });
    }
    //设置播放按钮动作
    ...
    private void setVolumeButton(){
        volumeBT.setOnAction((ActionEvent e)->{
            ...
            if(mediaPlayer.getVolume()>0){
                volumeValue = mediaPlayer.getVolume();
                volumeSD.setValue(0);
                setIcon(volumeBT,App.volOffIcon,25);
            }else{
                ...
                public void run() {
                    ...
                    if(!volumeSD.isValueChanging()){
                        ...
                        if(mediaPlayer.getVolume() == 0){        //设置音量按钮
                            setIcon(volumeBT,App.volOffIcon,20);
                        }else{
                            setIcon(volumeBT,App.volOnIcon,20);
                        }
                    }
                }
            });
